	                  ...
	                  string output_path)
{
	...
	for(int i = 0; i < keypoints_mem_current.size(); i++)
	{
    	...
    	if(i == 0)
    	{
		    ...
		    velocity_video.open(velocity_path, VideoWriter::fourcc('M',  'J', 'P', 'G'),  fps , frame.size(), true);
		...
		line(velocity_img, a, b, max_final, 1, LINE_AA);
		line(velocity_img, b, c, min_final, 1, LINE_AA);

		for(int j = 0; j < keypoints_mem_current[i].size(); j++)
		{
			if(valid[i][j] == true && !pt2.empty() && !pt1.empty())
			{
				line(velocity_img, pt1[j], pt2[j], velocity_mem[i][j], 1, LINE_AA);
			}
		}
